...
var OBJDoc = function (fileName) {
  ...
  this.vertices = new Array(0);  // Initialize the property for Vertex_
...
OBJDoc.prototype.parse = function (fileString, scale, reverse) {
  ...
  while ((line = lines[index++]) != null) {
    ...
    switch (command) {
      ...
      case 'mtllib':     // Read Material_ chunk
      ...
      case 'g':   // Read Object name
        ...
        continue; // Go to the next line
      case 'v':   // Read vertex
        var vertex = this.parseVertex_(sp, scale);
      ...
      case 'vn':   // Read normal
        ...
        continue; // Go to the next line
      case 'usemtl': // Read Material_ name
        currentMaterial_Name = this.parseUsemtl(sp);
        continue; // Go to the next line
      case 'f': // Read face
        var face = this.parseFace(sp, currentMaterial_Name, this.vertices, reverse);
...
OBJDoc.prototype.parseVertex_ = function (sp, scale) {
  var x = sp.getFloat() * scale;
  var y = sp.getFloat() * scale;
  var z = sp.getFloat() * scale;
  return (new Vertex_(x, y, z));
}
...
function onReadMTLFile(fileString, mtl) {
  ...
  while ((line = lines[index++]) != null) {
    ...
    switch (command) {
      case '#':
        continue;    // Skip comments
      case 'newmtl': // Read Material_ chunk
...
MTLDoc.prototype.parseRGB = function (sp, name) {
  ...
  var g = sp.getFloat();
  var b = sp.getFloat();
  return (new Material_(name, r, g, b, 1));
}

//------------------------------------------------------------------------------
// Material_ Object
//------------------------------------------------------------------------------
var Material_ = function (name, r, g, b, a) {
  this.name = name;
